...
//	data structure pointers
	...
	if (rec.header.len - sizeof(recordHeaderStruct) > 0) {
		...
		printf("The last header: len = 0x%X, type = 0x%X, num = %d\n", 
		    rec.header.len, rec.header.type, rec.header.num);
		break;
	    }
	}
...
//	    rec.header.num, rec.header.type, rec.header.len, rec.header.timestamp.time);
	...
	case REC_BEGIN:
		...
		ctime((time_t *) &rec.header.timestamp.time));
	    break;
	case REC_END:
...
int main(int argc, char **argv)
{
    ...
    if (argc < 0) {
	printf("Usage rhic2root filein.data fileout.root\n");
	return -1;
    }
    fin = fopen(argv[1], "rb");
    if (fin == NULL) {
