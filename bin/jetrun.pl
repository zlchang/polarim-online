...
# B00 = 2015
$yearcode = B00;
$basedir = "/home/polarim/polarim/bin";
$CheckPhysicsDelta = 600000;    # Check for "Physics Running" every xxx-ms

# Get today's date, we need it for the file name & run number...
($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) = localtime(time);
$nextday = sprintf("x%02d%02d",$mday,$mon+1);

# Reading from run number file
if (open(FNAM,".runname")) {
  $runname = <FNAM>;        # x1234.123
  close(FNAM);
  chomp($runname);      # remove the return (\n) character

  $runheader = substr($runname,0,5);
#  $runnumber = substr($runname,6,3);
#  $runnumber = $runnumber - $yearcode;
  $runnumber = "0".substr($runname,7,2);

# Has the day changed since the last time this was started?
  checkday();
}
else {
  ...
  $runheader = $nextday;
  $runnumber = 1;
  $n = $yearcode + $runnumber;
  $runname = "$runheader.$n";
  printf FNAM "$runname\n";
  close(FNAM);
}

$inidir = "/home/polarim/polarim/config/";
...
$frame
    ->Label(-text=>"RunHEADER:")
    ->pack(-side=>'left',-anchor=>'w');

$frame
    ->Entry(-justify=>'right',-textvariable=>\$runheader,-font=>'r16',
	    -width=>6)
    ->pack(-side=>'left',-anchor=>'w');
...
#----------------------------------------
$frame
    ->Label(-text=>"RunNumber:  $yearcode + ")
    ->pack(-side=>'left',-anchor=>'w');
$frame
    ->Entry(-justify=>'right',-textvariable=>\$runnumber,-font=>'r16',
	    -width=>4)
    ->pack(-side=>'left',-anchor=>'w');
...
# START BUTTON
#----------------------------------------
$start_b = $frame ->Button(-text => "Start Run", 
...
# STOP BUTTON
#----------------------------------------
$stop_b = $frame->Button(-text => "Stop Run", 
			 -activebackground=>'gray60',
			 -command=>\&popup_stop)
    ->pack(-side=>'left',-anchor=>'w');
...
#----------------------------------------
$frame
    ->Checkbutton(-text=>" Cycle ", 
        -variable=>\$autorestart,
        -indicatoron=>0,-pady=>5,
...
$frame1
    ...
    ->pack(-side=>'left',-anchor=>'w');
$frame1
    ->Entry(-textvariable=>\$inifile,-font=>'r16',
	    -background=>'gray80', -width=> 60)
    ->pack(-side=>'left',-pady=>2,-anchor=>'w');

$frame1->Button(-text => "Browse", 
	       -activebackground=>'gray60',
	       -command=>\&popup_file_browser)
...
$frame2
    ->Label(-text=>"Comment:  ")
    ->pack(-side=>'left',-pady=>1,-anchor=>'w');
$frame2
    ->Entry(-textvariable=>\$comment,-font=>'r16',
	    -background=>'gray80', -width=>60)
    ->pack(-side=>'left',-pady=>2,-anchor=>'w'); #, -expand=>1);

#----------------------------------------------------------
# Enable CheckPhysicsRunning Auto Sart/Stop Check Button
#----------------------------------------------------------
$frame2
    ->Checkbutton(-text=>" Auto Start/Stop ", 
        -variable=>\$ALPHA,
        -indicatoron=>0,-pady=>5,
        -selectcolor=>"LightGreen")
#		-offvalue=>1,
...
#----------------------------------------
# Tshift value - had no effect 3/10/09
#----------------------------------------
#$frame2
#    ->Label(-text=>"Tshift:  ")
#    ->pack(-side=>'left',-padx=>1,-anchor=>'w');
#$frame2
#    ->Entry(-textvariable=>\$tshift,-font=>'r16',
#	    -background=>'gray80', -width=>6)
#    ->pack(-side=>'left',-padx=>2,-anchor=>'w');
...
# Calculate ms until midnight and add 1000, just to make sure we get past it.
# At midnight, update the file name...
$msTill24 = 1000*(3600*(23-$hour) + 60*(60-$min)) + 1000;
$TleftToday = $mw->after($msTill24, \&midnight);
$autorestart = 0;
$ALPHA = 0;
...
# startrun
sub startrun {
  $commanddir = "/home/polarim/polarim/bin";
  $command = "./rhicpol -J -P -e0";
  $datafile = "/home/polarim/hjet/2015/data/$Runn.data";
  $logfile = "/home/polarim/hjet/2015/log/$Runn.log";

  $ENV{'HJET_ROOTFILE'} = "/home/polarim/hjet/2015/root/$Runn.root";
...
# Removed -T $tshift because had no efect
    $commandline = "cd $commanddir; $command -i $inifile -l $logfile -f $datafile -t $rtime -c \"$comment\" &";
...
# Update RUN FILE HEADER, if next day...
  checkday();
  $mw->idletasks;
...
# Update RUN FILE HEADER, if next day...
  ...
  ($sec, $min, $hour, $mday, $mon, $year, $wday, $yday, $isdst) = localtime(time) ;
  $n = $yearcode + $runnumber;
  $Runn = sprintf("%s.%03d", $runheader, $n);
...
sub popup_stop() {
  if ($running != 0) {
    ...
    $wstop->Label(-text=>" Do you want to stop this run now ? ")
	  ->pack(-side=>'top',-expand=>1, -fill=>none);
    $wstop->Button(-text=>"YES",
      -command=>sub{$wstop->withdraw; $autorestart=0; &stoprun();})
      ->pack(-side=>'top',-expand=>1, -fill=>none);
    $wstop->Button(-text=>"NO (continue measurement)",
      -command=>sub{$wstop->withdraw})
	  ->pack(-side=>'left',-expand=>1, -fill=>none);
  }
}
...
sub popup_exit() {
  if ($running != 0) {
    ...
    $wstop->Label(-text=>" Do you want to stop this run now ? ")
      ->pack(-side=>'top',-expand=>1, -fill=>none);
    $wstop->Button(-text=>"YES (Stop and Exit Now)",
      -command=>sub{$wstop->withdraw; $autorestart=0; &stoprun(); exit;})
      ->pack(-side=>'top',-expand=>1, -fill=>none);
    $wstop->Button(-text=>"NO (Continue Measurement)",
      -command=>sub{$wstop->withdraw})
      ->pack(-side=>'left',-expand=>1, -fill=>none);
...
# Check to see if physics running is on... 
 $PhysRunId->cancel;
 $sta = `./getCdevValue ringSpec.yellow ringStateS`;
 $sln = length($sta) - 1;
 if ($sln > 15) { $sln = 15; }
 $ystate = substr($sta, 0, $sln);

 $sta = `./getCdevValue ringSpec.blue ringStateS`;
 $sln = length($sta) - 1;
 if ($sln > 15) { $sln = 15; }
...
sub midnight() {
# Do nothing if a run is in progress, except schedule the next callback...
  if ($running == 0) {
    checkday();
    $mw->idletasks;
  }
...
#      elsif ($mline =~ m/RunID:/) {
#        $fillno = substr($mline,21,4);
#        $gev = substr($mline,33,6);
#      }
...
#      $mline = sprintf("%s %s %5d %s %s\n",$runname, $startstr, $eltime, $fillno, $gev);
      $mline = sprintf("%9s %5s %5d %5d %9.4e %9.4e %5.1f %5.1f %5.1f %5.1f %6.2f %5.1f %5.1f %7d %s\n",$runname, $startstr, $fillNumberM, $eltime, $wcmBeamMy, $wcmBeamMb, $yxpos, $yypos, $bxpos, $bypos, $beamEnergyM, $polarizationMy, $polarizationMb, $absolutePosition, $asymline);
      printf FSUM $mline;
    }
    close(FSUM);
  }
...
#
{
 ...
 $scdv = `./getCdevValue $CDEV_device_name $Parameter_name`;
 if (IsNumber($scdv)) {
  return $scdv;
 }
 else {
...
# Remove trailing CR & white space & leading white space
 ...
 if ($_[0] != 0) {  return 1; }
  ...
  if ($_[0] eq "0" or $_[0] eq "+0" or $_[0] eq "-0") { return 1; }
  else { return 0; }
 }                            
}
